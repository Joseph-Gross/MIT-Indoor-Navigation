...
int distance;
float direction_angle = 0; // offset from east, which I guess is to the right
float prev_direction_angle = 0;
int prev_distance = 0;
const int center_x = 128/2;
int center_y = 160/2; // only y center should change We don't need to fit that much on the screen
int LOOP_PERIOD=100;
bool navigating;
int32_t timer;
...
class Arrow {
  ...
  public:
    Arrow(TFT_eSPI* tft_to_use, int center_y = 80){
      center.x = center_x; // always 64
      center.y = center_y; // variable but probably 80
      local_tft = tft_to_use;
      color.r = 0;
      color.g = 0;
      color.b = 255;
      // now we have to calculate length. For now we'll just do
      length = 70;
      width = 12;
    }
    void update(bool navigating, float angle, int distance){
      if (navigating){
        ...
        local_tft->fillTriangle(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, ST7735_GREEN);
        local_tft->fillTriangle(p1.x, p1.y, p4.x, p4.y, p3.x, p3.y, ST7735_GREEN);
        local_tft->fillTriangle(p5.x, p5.y, p6.x, p6.y, p7.x, p7.y, ST7735_GREEN);
      }
      else {
        local_tft->fillScreen(BACKGROUND);
      }
    }
};
...
// Mag_x_offset = -7.22,Mag_y_offset = 658.77,Mag_z_offset = 287.20,Mag_x_scale = 1.30,Mag_y_scale = 1.05,Mag_z_scale = 0.78; // might be bad
// Mag_x_offset = -137.10,Mag_y_offset = 398.16,Mag_z_offset = -224.16,Mag_x_scale = 1.19,Mag_y_scale = 0.97,Mag_z_scale = 0.89;
// Mag_x_offset = -28.86,Mag_y_offset = 390.92,Mag_z_offset = -336.24,Mag_x_scale = 1.08,Mag_y_scale = 1.10,Mag_z_scale = 0.86;
// Mag_x_offset = -14.43,Mag_y_offset = 304.05,Mag_z_offset = -238.17,Mag_x_scale = 0.92,Mag_y_scale = 1.22,Mag_z_scale = 0.91;
float Mag_x_offset = -30.0,Mag_y_offset = 365.0,Mag_z_offset = -255.17,Mag_x_scale = 1.05,Mag_y_scale = 1.10,Mag_z_scale = 0.9;
// Mag_x_offset = -36.08,Mag_y_offset = 383.68,Mag_z_offset = -273.19,Mag_x_scale = 1.06,Mag_y_scale = 1.05,Mag_z_scale = 0.91;
...
void view_heading_LCD()
{
  ...
  */
  ...
  // ----- display the pitch
  tft.setCursor(0, 1);
  (pitch < 0) ? tft.print("P-") : tft.print("P+");
  tft.print((int(abs(pitch * 10)) / 1000));                // 1st digit
  tft.print((int(abs(pitch * 10)) / 100) % 10);            // 2nd digit
  tft.print((int(abs(pitch * 10)) / 10) % 10);             // 3rd digit
  tft.print(".");                                          // decimal point
  tft.print(int(abs(pitch * 10)) % 10);                    // last digit

  // ----- display the roll
  tft.setCursor(0, 15);
  (roll < 0) ? tft.print("R-") : tft.print("R+");
  tft.print((int(abs(roll * 10)) / 1000));                 // 1st digit
  tft.print((int(abs(roll * 10)) / 100) % 10);             // 2nd digit
  tft.print((int(abs(roll * 10)) / 10) % 10);              // 3rd digit
  tft.print(".");                                          // decimal point
  tft.print(int(abs(roll * 10)) % 10);                     // last digit

  // ----- display the heading
  tft.setCursor(0, 30);
  tft.print("Heading ");                                   // Heading
